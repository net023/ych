$(function() {
	...
	$('#addButton').click(function(){
		...
		$('#addForm').attr('action', 'mtser_cost/addOrUpdate')._ajaxForm(function(r){
			if(r.r){
				$('#grid').datagrid('reload');
			}else{$.messager.alert('操作提示', r.m,'error');}
		});
	...
	function handler_add() {
        $('#editForm').attr('action','mtser_cost/add').resetForm();
        $('#id').val('');
        ...
        if(check.length > 0){
            $.messager.confirm('操作提示', '确定要删除过滤品牌？', function(r){
                if (r){
                    ...
                    $._ajaxPost('mtser_cost/batchDel',{ids:ids.join('|')},function(r){
...
var formatter = {
    opt : function(value, rowData, rowIndex) {
        var html = '<a class="spacing a-red" onclick="delVersion('+rowIndex+');" href="javascript:void(0);">删除</a>';
        return html;
    }
};
...
function updVersion(rowIndex) {
    $('#editForm').attr('action','mtser_cost/update').resetForm();
...
function delVersion(rowIndex) {
    var data = $('#grid').datagrid('getRows')[rowIndex];
    $.messager.confirm('操作提示', '确定要删除该保养项目工时费？', function(r){
        if (r){
        	$._ajaxPost('mtser_cost/batchDel',{ids:data.ms_id},function(r){
        		if(r.r){$('#grid').datagrid('reload');}else{$.messager.alert('操作提示', r.m,'error');}
        	});
        }
    });
