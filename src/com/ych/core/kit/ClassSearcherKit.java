...
public class ClassSearcherKit {
	...
	protected static final Logger LOG = Logger.getLogger(ClassSearcherKit.class);
	...
	private static <T> List<Class<? extends T>> extraction(Class<T> clazz, List<String> classFileList) {
		...
		for (String classFile : classFileList) {
			Class<?> classInFile = ReflectKit.on(classFile).get();
	...
	public static ClassSearcherKit of(Class target) {
		return new ClassSearcherKit(target);
	}
		...
		if (!baseDir.exists() || !baseDir.isDirectory()) {
			...
			for (int i = 0; i < filelist.length; i++) {
				...
				if (readfile.isDirectory()) {
					...
					if (ClassSearcherKit.wildcardMatch(targetFileName, tempName)) {
						String classname;
	...
	@SuppressWarnings("rawtypes")
	public ClassSearcherKit(Class target) {
		this.target = target;
	}

	public ClassSearcherKit injars(List<String> jars) {
		if (jars != null) {
	...
	public ClassSearcherKit inJars(String... jars) {
		if (jars != null) {
			for (String jar : jars) {
				includeJars.add(jar);
			}
	...
	public ClassSearcherKit classpath(String classpath) {
	...
	public ClassSearcherKit includeAllJarsInLib(boolean includeAllJarsInLib) {
		this.includeAllJarsInLib = includeAllJarsInLib;
		return this;
	}

	public ClassSearcherKit libDir(String libDir) {
		this.libDir = libDir;
		return this;
	}
